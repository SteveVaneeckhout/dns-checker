...
export function typeNumber(name) {
    const n = TYPE_NUMBERS[name.toUpperCase()];
    if (n === undefined)
        throw new Error(`unknown RR type: ${name}`);
    return n;
}
...
export function encodeIPv4(addr) {
...
export function encodeIPv6(addr) {
    let parts;
    if (addr.includes("::")) {
...
export function encodeSoaRdata(d) {
    return Buffer.concat([
        encodeName(d.mname),
        encodeName(d.rname),
...
export function encodeRrsigRdataForSigning(r) {
    return Buffer.concat([
...
export function encodeRdata(answer) {
    switch (answer.type) {
        case "A":
...
export function encodeRrForSigning(rrset, rrsig) {
    if (rrset.length === 0)
        throw new Error("cannot sign empty RRset");
    const ownerName = rrset[0]?.name;
    if (ownerName === undefined)
        throw new Error("RRset missing name");
...
export function computeDsDigest(ownerName, dnskey, digestType) {
    ...
    if (digestType === 4)
        return createHash("sha384").update(input).digest();

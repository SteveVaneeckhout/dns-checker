...
export async function validateChain(host, dns, anchors = ROOT_TRUST_ANCHORS) {
  const result = {
    signed: false,
    chainValid: false,
    chain: [],
  ...
  try {
    ...
    const rootSigOk = verifyRrsig(
      root.rrsig,
      root.rrset.map((a) => ({ ...a, data: a.data })),
      rootKeyData,
    );
    if (!rootSigOk) {
...
export async function verifyRecord(type, name, dnskeys, dns) {
  try {
    ...
    if (!got.rrsig) return { verified: false, answers: got.rrset, error: "no RRSIG" };

...
function parseRsaKey(key) {
    if (key.length < 1)
        throw new Error("RSA DNSKEY too short");
    let offset = 0;
    let expLen = key[0] ?? 0;
    offset = 1;
    if (expLen === 0) {
    ...
    if (key.length < offset + expLen + 1)
        throw new Error("RSA DNSKEY truncated");
    const exponent = key.subarray(offset, offset + expLen);
    const modulus = key.subarray(offset + expLen);
    return { modulus, exponent };
}
function makeRsaPublicKey(key) {
...
function makeEcdsaPublicKey(key, curve) {
    const expected = curve === "P-256" ? 64 : 96;
    if (key.length !== expected)
        throw new Error(`ECDSA ${curve} DNSKEY wrong size`);
...
function makeEd25519PublicKey(key) {
    if (key.length !== 32)
        throw new Error("Ed25519 DNSKEY wrong size");
    return createPublicKey({
        key: { kty: "OKP", crv: "Ed25519", x: b64url(key) },
        format: "jwk",
    });
}
export function verifyRrsig(rrsig, rrset, dnskeys) {
    const candidates = dnskeys.filter((k) => computeKeyTag(k) === rrsig.keyTag && k.algorithm === rrsig.algorithm);
    if (candidates.length === 0)
        return false;
    const data = rrsigSigningInput(rrsig, rrset);
    for (const key of candidates) {
        try {
            if (verifyOne(rrsig, data, key))
                return true;
        }
        catch {
...
function verifyOne(rrsig, data, dnskey) {
    switch (rrsig.algorithm) {
        ...
        case 13:
            return cryptoVerify("sha256", data, { key: makeEcdsaPublicKey(dnskey.key, "P-256"), dsaEncoding: "ieee-p1363" }, rrsig.signature);
        case 14:
            return cryptoVerify("sha384", data, { key: makeEcdsaPublicKey(dnskey.key, "P-384"), dsaEncoding: "ieee-p1363" }, rrsig.signature);
        case 15:
